...
if [[ -z "$PAYLOAD_NAME" ]]; then
  echo "SR_BRIDGE: missing payload name"
  exit 2
fi
...
echo "bridge_apply_${PAYLOAD_NAME}" > "$STATE_DIR/bridge.last_phase"

if [[ ! -f "$PAYLOAD_DIR/index.js" ]]; then
  echo "SR_BRIDGE: missing payload index.js at $PAYLOAD_DIR"
  exit 2
fi
...
if ! systemctl is-active --quiet volumio; then
  echo "SR_BRIDGE: volumio did not recover after installing $PAYLOAD_NAME"
  exit 2
fi

echo "SR_BRIDGE: applied Bridge payload $PAYLOAD_NAME"
